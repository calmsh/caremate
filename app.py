...
# ---------------------------------------------------------
OPENAI_API_KEY = "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"
 ...
 with st.container():
  ...
  if final_input:
   ...
   try:
    ...
    sys_msg = (f"당신은 전문적인 건강 상담사입니다. "
               f"상담 대상: {u['name']}님({u['age']}세, {u['gender']}). "
               f"현재 위험 상태: {st.session_state.risks_summary}. "
               f"정신건강 상태: 우울 {phq}점, 불안 {gad}점. "
               f"환자에게 친절하고 전문적인 예방 조언을 구체적으로 제공하세요.")
    
    res = llm.invoke([SystemMessage(content=sys_msg), HumanMessage(content=final_input)])
    ai_text = res.content
    ...
    st.session_state.chat_history.append({"role": "ai", "content": ai_text, "audio": audio_fp.getvalue()})
    st.rerun()
   except Exception as e:
    st.error(f"상담 중 오류가 발생했습니다: {e}")

  if st.button("⬅ 결과 리포트로 돌아가기"): st.session_state.step = 3; st.rerun()
  st.markdown('</div>', unsafe_allow_html=True)